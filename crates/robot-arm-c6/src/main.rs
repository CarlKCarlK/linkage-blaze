...
fn inner_main() -> Result<Infallible, MainError> {
    ...
    esp_println::println!("c6: entering game loop with joystick reporting");
    loop {
        let vrx = loop {
        ...
        let joystick_changed = cyd_sim.set_lower_arm_and_spin_whole(vry01, vrx01);
        ...
        match cyd_sim.tick(Instant::now(), None) {
            TickOut::Draw => {
