...
pub fn draw_calibration_cross<E>(
    ...
    let center = calibration_corner_center(calibration_corner);
...
fn solve_3x3(system_matrix: [[f32; 3]; 3], rhs_vector: [f32; 3]) -> (f32, f32, f32) {
    ...
    assert!(
        determinant.abs() >= AFFINE_DETERMINANT_EPSILON,
...
mod tests {
    use super::{
        CALIBRATION_POINT_COUNT, CalibrationConfig, CalibrationCorner, calibration_corner_center,
