...
if (initialSourceResult.errorMessage) {
  startupErrorMessage = initialSourceResult.errorMessage;
}

const editor = new EditorView({
  doc: initialSourceResult.source,
  extensions: [
    editorSetup,
    rust(),
    oneDark,
    ...
    EditorView.updateListener.of((update) => {
      if (update.docChanged) {
        ...
        // Autosave remains a single slot, so editing a #code= link replaces the
        // prior autosaved document just like opening a file does.
...
function loadInitialSource() {
  const sourceFromFragmentResult = decodeSourceFromHash(location.hash);
  if (sourceFromFragmentResult.source !== null) {
    return sourceFromFragmentResult;
  }

  return {
    source: localStorage.getItem(STORAGE_KEY) ?? default_program(),
    errorMessage: sourceFromFragmentResult.errorMessage,
  };
}

function syncErrorMessage() {
  error.textContent = [startupErrorMessage, renderErrorMessage].filter(Boolean).join("\n");
}
...
function decodeSourceFromHash(hash) {
  ...
  const encodedSource = hash.slice(CODE_FRAGMENT_PREFIX.length);
  try {
    return {
      source: decodeCodeFragment(encodedSource),
      errorMessage: "",
    };
  } catch (caught) {
    return {
      source: null,
      errorMessage: `Invalid #code fragment: ${String(caught)}`,
    };
  }
}
...
function updatePreview() {
  try {
    ...
    renderErrorMessage = String(caught);
    syncErrorMessage();
  }
}
...
async function copyLink() {
  const url = `${location.origin}${location.pathname}${CODE_FRAGMENT_PREFIX}${encodeCodeFragment(getSource())}`;
  try {
    await navigator.clipboard.writeText(url);
    clearTimeout(copyLinkFeedbackTimer);
    copyLinkButton.textContent = COPIED_LINK_LABEL;
    copyLinkFeedbackTimer = setTimeout(() => {
      copyLinkButton.textContent = COPY_LINK_LABEL;
    }, 1500);
  } catch (caught) {
    renderErrorMessage = `Failed to copy link: ${String(caught)}`;
    syncErrorMessage();
  }
}
